...
| POST | `/auth/login` | Public | Local email/password login. Sets the central cookie. `401` invalid credentials; `403` inactive user. Accounts created with a temporary password authenticate only into the forced password-change flow until it is replaced. |
| GET | `/auth/csrf` | Session | Returns a short-lived, session-bound token for `X-CSRF-Token` on cookie-authenticated mutations. |
| GET | `/auth/me` | Session | Current profile and global roles, plus `is_local_account` and `must_change_password`. App-scoped roles are intentionally excluded. |
| PATCH | `/auth/me` | Local session + CSRF | Updates the caller's display name. External-provider accounts cannot use this endpoint. |
| POST | `/auth/me/password` | Local session + CSRF | Changes the caller's local password and clears `must_change_password`; requires the current password. |
...
| POST | `/users/` | Creates an active Orbita-local user from `{email,full_name}` and returns a one-time generated `temporary_password`. The account must change it at first login. |
